...
namespace QuickOrderApp.ViewModels.LoginVM
{
    public class LoginViewModel : BaseViewModel
    {
        ...
        public LoginViewModel(INavigation _navigation)
        {
            ...
            DoneCommand = new Command(async () => await DoneCommandHandler());
        ...
        private bool CheckNullOrEmptyProperties(string fullname, string email, string username, string password, string confirmpassword, string phone, string adress, string gender)
        {
        ...
        private bool CheckEmailPatter(string emailValue)
        {
        ...
        private void CancelLoginAction()
        {
                ...
                if (!ConfirmAndPasswordValidator.HasError)
                {
                    if (!await userDataStore.CheckIfUsernameAndPasswordExist(Username, Password))
                    {
                        ...
                        await this.authService.SendEmailVerificationCode(newUser.Email).ContinueWith((task) =>
                        {
                            ...
                            {
                                App.Current.MainPage.DisplayAlert("Error Email", "Can not send confirmation email", "OK");
                            }
                        });
                        //TODO: SEND EMAIL
                        //try
